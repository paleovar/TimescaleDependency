...
ylab_forc_s <- setNames(c(TeX("RF $(W/m^{2})$"), TeX(" conc. $(ppm)$"), TeX("AOD"), TeX(" RF $(W/m^{2})$"), TeX(" $\\Delta$ RF $(W/m^{2})$")), c("sol", "ghg", "vol", "orb", "lu"))
leg_forc <- setNames(c(TeX("TSI"), TeX("CO$_2$"), TeX("volcanic"), TeX("insolation $65°$N"), TeX("land use")), c("sol", "ghg", "vol", "orb", "lu"))
...
ggdraw(plotgrd) + 
  draw_label(TeX("solar"), x = 0.98, y = 0.63, color=cut_colors[["sol"]], angle=-90,  size=textsize) +
  draw_label(TeX("volcanic"), x = 0.98, y = 0.88, color=cut_colors[["vol"]], angle=-90,  size=textsize) +
  draw_label(TeX("CO$_2$"), x = 0.98, y = 0.4, color=cut_colors[["ghg"]], angle=-90,  size=textsize)  +
  draw_label(TeX("insolation $65°$N"), x = 0.98, y = 0.2, color=cut_colors[["orb"]], angle=-90,  size=textsize) 
...
for(n in add_forc){
  if(n=="vol"){next}
    t <-  theme(legend.position = c(0.85,0.2),
                ...
                legend.background = element_blank(),
                legend.box.background = element_blank(),
                legend.text = element_text(color=cut_colors[[n]], size=textsize),
                axis.text.y = element_text(margin = unit(c(t = 0, r = 2.5, b = 0, l = 0), "mm"), colour=cut_colors[[n]]),
                axis.text.x = element_text(margin = unit(c(t = 2.5, r = 0, b = 0, l = 0), "mm"), colour="black"),
                axis.ticks.y = element_line(colour=cut_colors[[n]]),
                axis.ticks.length = unit(-1.4, "mm"),
...
y.grob <- textGrob(TeX('radiative forcing $(W/m^{2})$'), 
                   gp=gpar(col="black", fontsize=textsize), rot=90)
y.top <- textGrob(TeX(' '), 
                            gp=gpar(col="black", fontsize=textsize))
...
ggdraw(plotgrd) + 
  draw_label(TeX("solar"), x = 0.98, y = 0.75, color=cut_colors[["sol"]], angle=-90,  size=textsize) +
  draw_label(TeX("CO$_2$"), x = 0.98, y = 0.31, color=cut_colors[["ghg"]], angle=-90,  size=textsize)
