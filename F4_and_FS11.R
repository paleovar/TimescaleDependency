...
#wrapper for statistical measures irr::agree() and irr::kappa2
kappa.stat <- function(target1, target2, method){
  ...
  if(method=="agreement"){ #categorical agreement
    rating <- data.frame(
  ...
  if(method=="kappa"){ #interrater reliability
    rating <- cbind(tmp$data[[1]]$category, tmp$data[[2]]$category)
    return(kappa2(rating)$value)
  }
}

#compute overlap between different mean values with standard deviations
...
#compute agreement based on the interrater reliability and categorical agreement
...
for(method in c("agreement", "kappa")){ 
  #l <-list()
...
#merge results from all statistical measures and plot 
...
tbb <- tbb %>% add_column(method2= factor(.$method, rev(c("percent agree wi margins", "agreement", "kappa"))))
ggplot(tbb %>% filter(!alt_name2=="pages2k")) + 
